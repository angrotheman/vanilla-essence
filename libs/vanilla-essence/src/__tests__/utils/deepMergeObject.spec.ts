import { deepMergeObject } from '../../lib/utils/deepMergeObject';
describe('objectMerge test', () => {
  it('should replace true with false', () => {
    const objA = { a: true };
    const objB = { a: false };
    expect(deepMergeObject(objA, objB)).toEqual({ a: false });
  });

  it('should merge two simple objects', () => {
    const objA = { a: true };
    const objB = { b: 0 };
    expect(deepMergeObject(objA, objB)).toEqual({ a: true, b: 0 });
  });

  it('should merge two objects with basic deep level', () => {
    const objA = { a: true, c: { zoom: 12 } };
    const objB = { b: 0 };
    expect(deepMergeObject(objA, objB)).toEqual({
      a: true,
      c: { zoom: 12 },
      b: 0,
    });
